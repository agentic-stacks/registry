...
CATEGORY_ORDER = ["platform", "storage", "hardware", "networking", "automation", "observability", "security", "other"]
...
def load_stacks(stacks_dir: pathlib.Path) -> list[dict]:
    ...
    stacks.sort(key=lambda s: (
        CATEGORY_ORDER.index(s.get("category", "other")) if s.get("category", "other") in CATEGORY_ORDER else 99,
        s["name"],
    ))
    return stacks
...
def build_main_table(stacks: list[dict]) -> str:
    ...
    for s in stacks:
        ...
        lines.append(f"| [{name}](https://www.agentic-stacks.com/stacks/{owner}/{name}) | {target} | {skills} |")
    return "\n".join(lines)


def update_between_markers(content: str, marker_start: str, marker_end: str, replacement: str) -> str:
...
def main():
    ...
    if args.dotgithub:
        ...
        if readme.exists():
            ...
            updated = update_between_markers(content, "<!-- STACKS-TABLE-START -->", "<!-- STACKS-TABLE-END -->", table)
            if updated != content:
